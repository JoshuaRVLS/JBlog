...
export function setupSocketHandlers(io: Server): void {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    // Send message (optimized for real-time - emit before DB save)
    socket.on("send-message", async (data: { 
      ...
      try {
        ...
        // Verify user is member (quick check)
        ...
        // Get all group members to match mentions (parallel with message creation)
        const groupMembersPromise = db.groupChatMember.findMany({
        ...
        // OPTIMIZATION: Emit optimistic message IMMEDIATELY before DB save for instant feedback
        ...
        // Broadcast pesan ke semua member group IMMEDIATELY (before DB save)
        io.to(`group:${groupId}`).emit("new-message", optimisticMessage);

        // Get group members for mentions (await after emit)
        const groupMembers = await groupMembersPromise;
        ...
        // Save ke database (non-blocking, run in background)
        ...
        Promise.all([
          saveMessagePromise.then((message) => {
          ...
          // Buat notifikasi untuk user yang di-mention (non-blocking)
          Promise.all(
            mentionedUserIds.map(async (mentionedUserId) => {
              try {
                const message = await saveMessagePromise;

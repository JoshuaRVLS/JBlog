...
export const loginUser = async (req: Request, res: Response) => {
    try {
        ...
        if (!user || !(await bcrypt.compare(password, user.password))) {
            return res.status(StatusCodes.UNAUTHORIZED).json({msg: 'Wrong Email or Password'});
        }
...
export const refreshToken = async (req: Request, res: Response) => {
    try {
        ...
        if (!storedRefreshToken) {
            return res.status(StatusCodes.UNAUTHORIZED).json({msg: 'Refresh Token Expired'});
        }

        if (refreshToken !== storedRefreshToken.value) {
            return res.status(StatusCodes.UNAUTHORIZED).json({msg: 'Refresh Token Invalid'});
        }

        const newAccessToken = await encrypt({id: storedRefreshToken.userId}, '15m');
